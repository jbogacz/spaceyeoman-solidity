...
contract('SpaceLand', ([_, yopman, buyer1st, buyer2nd]) => {
  ...
  context('single buy', () => {
    describe('lands are bought', () => {
      ...
      it('should revert for invalid payment value', async () => {
      ...
      it('should return multiple owned land ids', async () => {
        ...
        ownedLandIds.length.should.be.equal(2);
  ...
  context('multiple buy', () => {
    ...
    it('should revert for incorrect total payemnt value', async () => {
      await shouldFail.reverting(this.spaceLand.buyMany([1, 2, 3], { value: PRICE }));
    });

    it('should revert for ids length greater than 10', async () => {
      const landIds = Array.apply(null, {length: 11}).map(Number.call, Number);
      await shouldFail.reverting(this.spaceLand.buyMany(landIds, { value: PRICE }));
    });

    it('should process buy with correct total payment value', async () => {
      const landIds = Array.apply(null, {length: 10}).map(Number.call, Number);
      await this.spaceLand.buyMany(landIds, { value: 10 * PRICE });
    })
  });
});
